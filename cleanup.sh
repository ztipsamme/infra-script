...
az sql db delete -g $RESOURCE_GROUP -s $SQL_SERVER_NAME -n $SQL_DB_NAME -y
az sql server delete -g $RESOURCE_GROUP -n $SQL_SERVER_NAME -y

az monitor app-insights component delete -g $RESOURCE_GROUP -a $APP_INSIGHT_NAME
...
PROJECT_ID=$(az devops project show --project $APP_NAME --query id -o tsv)
